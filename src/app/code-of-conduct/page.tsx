...
const BEHAVIOR_DATA: BehaviorData[] = [
  {
    id: "01",
    title: "Be Considerate",
    tag: "EMPATHY",
    desc: "Your work affects users and colleagues. Take consequences into account when making decisions.",
  },
  {
    id: "02",
    title: "Be Patient",
    tag: "GROWTH",
    desc: "Patience helps build empathy towards others within the learning environment.",
  },
  {
    id: "03",
    title: "Be Respectful",
    tag: "SAFETY",
    desc: "Respect helps us feel safe and empowered to express our ideas freely.",
  },
  {
    id: "04",
    title: "Be Nice",
    tag: "KINDNESS",
    desc: "Treat others as you would like to be treated in your quest for knowledge.",
  },
  {
    id: "05",
    title: "Communicate",
    tag: "CLARITY",
    desc: "Effective communication fosters good working relationships with all peers.",
  },
  {
    id: "06",
    title: "Ask for Help",
    tag: "SUPPORT",
    desc: "Don't be afraid to ask for help when unsure; we learn together.",
  },
];

const UNACCEPTABLE_BEHAVIORS: string[] = [
...
const CONSEQUENCES: Consequence[] = [
  {
    level: "01",
    type: "Warning",
    desc: "Public or private explanation of negative impact.",
    final: false,
  },
  {
    level: "02",
    type: "Suspension",
    desc: "Temporary ban from all future community activities.",
    final: false,
  },
  {
    level: "03",
    type: "Permanent Ban",
    desc: "Total removal from the SLIIT Women In FOSS community.",
    final: true,
  },
];
...
export default function CodeOfConduct(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const unacceptableItems = gsap.utils.toArray<HTMLElement>(
        ".unacceptable-item",
      );
      ...
      const reportingElements = gsap.utils.toArray<HTMLElement>(
        ".reporting-element",
      );
  ...
  // Inline styles moved to a constant to avoid template literals in JSX
  const inlineStyles = `
      ...
      <div className="max-w-[1400px] mx-auto px-6 relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div className="lg:col-span-3">
              ...
              <p className="text-[10px] font-mono text-purple-400 uppercase tracking-widest leading-loose">
                Scope: Forums, Wiki, Repositories, IRC, Private Correspondence,
                and Public Meetings.
              </p>
            </div>
            <div className="lg:col-span-9">
              <h2 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter mb-8 leading-none">
                ...
                <br className="hidden md:block" /> every interaction.
              </h2>
              <div className="space-y-6 text-xl md:text-3xl font-light text-purple-100/60 leading-tight max-w-5xl">
                <p>
                  The purpose of our Code of Conduct is to ensure all
                  participants have the best possible experience. Diversity is
                  our strength, but it requires ground rules to ensure we remain
                  an excellent space for collaboration.
                </p>
                <p className="text-lg md:text-xl italic border-t border-white/10 pt-6">
                  &quot;Take it in the spirit in which it&apos;s intended &mdash;
                  a guide to make it easier to be excellent to one another.
        ...
        <section className="reveal-section mb-40">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/10 border border-white/10">
            {BEHAVIOR_DATA.map((item) => (
              <div
                key={item.id}
                className="behavior-card p-10 bg-[#0f0720] md:hover:bg-white/[0.02] transition-colors group active:bg-white/[0.03] touch-manipulation"
              >
                <span className="text-[10px] font-mono text-purple-500 block mb-4 tracking-widest uppercase">
                  {item.tag} // {item.id}
                </span>
                <h3 className="text-2xl font-black uppercase italic mb-4 md:group-hover:text-purple-400 transition-colors">
                  {item.title}
                </h3>
                <p className="text-purple-100/50 font-light leading-relaxed">
                  {item.desc}
                </p>
              </div>
            ))}
        ...
        <section className="reveal-section mb-40">
          <div className="flex items-center gap-4 mb-12">
            ...
            <h2 className="text-4xl font-black uppercase italic tracking-tighter text-red-500">
          ...
          <div className="space-y-0 border-t border-white/10">
            {UNACCEPTABLE_BEHAVIORS.map((text, i) => (
              <div
                key={`${text.substring(0, 20)}-${i}`}
                className="unacceptable-item group border-b border-white/10 py-8 flex items-start md:items-center gap-4 md:gap-8 md:hover:bg-red-500/[0.02] transition-all active:bg-red-500/[0.03]"
              >
                <span className="text-3xl md:text-4xl font-black text-white/5 md:group-hover:text-red-500/20 transition-colors italic flex-shrink-0">
                  0{i + 1}
                </span>
                <p className="text-lg md:text-2xl font-light text-purple-100/70 md:group-hover:text-white transition-colors uppercase italic tracking-tight">
                  {text}
                </p>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="reporting-element lg:col-span-8 p-8 md:p-12 border border-purple-500/20 bg-purple-500/[0.02]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                ...
                <div className="p-6 bg-white/[0.03] border border-white/5">
                  ...
                  <p className="text-sm font-light text-purple-100/70">
                    You will receive an email immediately. We promise action
                    within{" "}
              ...
              <div className="flex flex-col md:flex-row gap-8 items-start md:items-center pt-8 border-t border-white/10">
                <a
                  href="mailto:infowifsliit@gmail.com"
                  className="text-xl md:text-2xl font-bold hover:text-purple-400 transition-colors break-all"
                >
                ...
                >
                  ...
                  <Send
                    size={14}
                    className="md:group-hover:translate-x-2 transition-transform"
                  />
                </a>
              </div>
            </div>
                ...
                {CONSEQUENCES.map((c) => (
                  <div key={c.level} className="border-b border-white/5 pb-4">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span
                        className={`text-xs font-black uppercase ${c.final ? "text-red-500" : "text-white"}`}
                      >
                        {c.type}
                      </span>
                    </div>
                    <p className="text-xs text-purple-100/40 font-light">
                      {c.desc}
                    </p>
                  </div>
                ))}
